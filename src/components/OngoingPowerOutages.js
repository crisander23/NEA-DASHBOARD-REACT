...
import '../css/OngoingPowerOutages.css'; // Import the new CSS specific to OngoingPowerOutages


const OngoingPowerOutages = () => {
  ...
  useEffect(() => {
    const fetchOngoingOutages = async () => {
      try {
        const response = await fetch('http://neapc04:3010/api/dashboard/retrieve-ongoing-power-outages/');
            ...
            <tbody>
              {outages.map((outage, index) => (
                <tr key={index}>
                  ...
                  <td style={{ width: '15%' }}>{outage.timeoff}</td>
                  <td style={{ width: '20%' }}>{outage.cause}</td>
...
export default OngoingPowerOutages;
