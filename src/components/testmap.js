...
const PhilippinesECMap = () => {
  ...
  useEffect(() => {
    // Create a map with a basic basemap
    const map = new Map({
      basemap: 'dark-gray-vector', // You can adjust the basemap here
    });

    // Create a view and center it over the Philippines
    const view = new MapView({
      container: 'viewDiv', // This is the id of the div where the map will render
      map: map, // The map created above
      center: [121.774, 12.8797], // Coordinates of the Philippines
      zoom: 6, // Adjust the zoom level for a regional view
    });
    ...
    // Function to create a circle for each location
    ...
    locations.forEach(location => {
      const { longitude, latitude, province } = location;
      const marker = createCircleMarker(longitude, latitude, province);
      graphicsLayer.add(marker); // Add marker to the graphics layer
      console.log(`Adding marker for ${province} at [${latitude}, ${longitude}]`);
    });
